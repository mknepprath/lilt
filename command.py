...
def get(tweet, inventory, id, position):
    ...
    if (a == 'drop'):
        ...
        if db.select('name', 'items', 'name', b) != None:
            return (True, item.drop(b, inventory, id, quantity=quantity))
    ...
    elif (a == 'inventory') or (cmdtweet == 'check inventory') or (cmdtweet == 'what am i holding'): # multi-word commands won''t work right now... a is only one word
        if inventory == {}:
